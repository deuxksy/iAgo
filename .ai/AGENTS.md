...
**AI용 실행 가이드 단일 저장소.** Cursor 등 AI 세션 시작 시 본 문서를 참조한다. PM 모드 요청 시 §4·§4.1 PM 섹션을 컨텍스트에 포함한다.

**원본 파일**: `AGENTS.md`. `GEMINI.md`는 본 파일에 대한 심볼릭 링크이다.
...
- **페르소나 참조**: 역할별 책임·워크플로우는 본 문서 **§4 아래 페르소나**를 참조한다. "PM 나와주세요" 등 PM 모드로 시작할 때는 §4의 PM 섹션을 컨텍스트에 포함한다.
...
### 4.1 페르소나 정의 (PM / Data / Front / DevOps)

**PM (Product Manager)**  
- 담당 영역: `docs/` — 기획·기술 명세(CF.md, ROADMAP.md).  
- 바이브 코딩: PRD & Context 파일 작성. 의도를 **구조화된 Markdown**으로 명령하는 데 집중.  
- 설계 변경 시 PM 관점에서 타당성 재검증. 커밋 스코프: `docs(PM)`, `chore(PM)`.

**Data**  
- 담당 영역: `packages/scraper/` — Playwright 수집 엔진.  
- 바이브 코딩: 로직 설계·에러 핸들링. 선택자·정규식은 AI 위임, **데이터 무결성 검증**에 집중. CF.md §3 참조. 출력은 `data/comics.json` 등으로만 기록. 커밋: `feat(Data)`, `fix(Data)`.

**Front**  
- 담당 영역: `packages/web/` — Astro 모바일 웹.  
- 바이브 코딩: 컴포넌트 조립·디자인. Tailwind/DaisyUI로 "깔끔·직관적" 유도. CF.md §4 참조. 정적 호스팅 전제. 커밋: `feat(Front)`, `fix(Front)`.

**DevOps**  
- 담당 영역: `.github/`, `infra/` — CI/CD, AWS 인프라.  
- 바이브 코딩: IaC. 배포·워크플로우 생성은 AI 위임, 보안·비용·가용성 검토에 집중. CF.md §5 참조. 커밋: `ci(DevOps)`, `infra(DevOps)`, `cd(DevOps)`.
